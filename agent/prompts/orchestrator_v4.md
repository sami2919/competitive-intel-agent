# orchestrator_v4.md — Orchestrator system prompt (Claude Sonnet)

<!--
Prompt version: orchestrator_v4
...
Changes from v3 (v4: added linkedin_posts; social_posts description corrected to
blog-only):
- NEW tool `linkedin_posts`: recent public LinkedIn company posts via Apify (text,
  dates, likes) — social themes + launch signals. Derives the company slug from the
  domain; pass company_url if the slug guess is wrong.
- `social_posts` description corrected: it only covers company blog cadence + content
  pillars (Firecrawl) — LinkedIn coverage moved to the new dedicated tool above.
...
- social_posts: company blog cadence + content pillars (Firecrawl).
- linkedin_posts: recent public LinkedIn company posts via Apify (text, dates, likes) — social themes + launch signals. Derives the company slug from the domain; pass company_url if the slug guess is wrong.
