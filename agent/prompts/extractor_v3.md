# extractor_v3.md — Bulk claim extraction prompt (Claude Haiku)

<!--
Prompt version: extractor_v3
Used by: agent/extractor.py
Records itself in: Claim.extracted_by = "haiku-4-5/extractor_v3"

Changed from v2 (output usefulness pass): adds rule 6 — a marketing-strategy
relevance filter. Live BambooHR run showed the appendix polluted with employer-brand
fluff ("founded in 2008", "award-winning culture", "inclusive work environment")
extracted from careers pages: true, sourced, and useless to a campaign writer.
...
6. Extract only claims relevant to MARKETING STRATEGY: positioning, messaging,
   pricing, ICP/segment targeting, campaigns, product-launch narrative, competitive
   proof points (ratings, customer counts). SKIP employer-brand and company-trivia
   assertions with no campaign relevance — founding year, office culture, awards for
   workplace, DEI statements, generic mission/vision boilerplate — UNLESS the page
   frames them as buyer-facing positioning. A hiring page's ROLE MIX (e.g. "hiring
   5 enterprise AEs") is signal; its culture copy is not.
